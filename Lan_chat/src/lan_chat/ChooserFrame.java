...
public class ChooserFrame extends JFrame {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (host != null && !host.isEmpty()) {
                    ...
                    if (!"".equals(message)) {
                        ...
                        Transporter.sendMessage(message, sendTime, host, Config.LocalHostName, Config.LocalHostIP);
                        messageArea.append(Config.LocalHostName + "  :  " + Config.LocalHostIP + "    " + sendTime +
                            "\r\n    " + message + "\r\n");
                    }
    ...
    public void setLocalHostInfo() {
        ...
        /*
        if(Config.LocalHostIP == null){
            //get all ip address available
            List<String> ips = Transporter.getWlanIP();
            
            //create a dialog to ask for ip address
            JComboBox ipselection = new JComboBox(ips.toArray());
            JComponent[] arrr = new JComponent[]{
                new JLabel("Please enter choose your ip address from this list:"),
                ipselection
            };
            
            int result = JOptionPane.showConfirmDialog(null,arrr,"lala",JOptionPane.PLAIN_MESSAGE);
            if (result == JOptionPane.OK_OPTION) {
                Config.LocalHostIP = String.valueOf(ipselection.getSelectedItem());
            } else {
                Config.LocalHostIP = null;
            }
        }
        */
        
        Config.LocalHostIP = Transporter.getWlanIP().get(0);
        
        try {
            Config.LocalHostName = InetAddress.getLocalHost().getHostName();
    ...
    public void showInformation(JTextArea textArea) {
        ...
        textArea.append("    " + Config.LocalHostName + "\r\n");
        textArea.append("    " + String.join("\r\n    ", Transporter.getWlanIP()));
        textArea.append("\r\n");
        //textArea.append("    " + Config.LocalHostIP + "\r\n");
