...
public class Transporter {
        ...
        new Thread(new Runnable() {
            public void run() {
                try (Socket s = new Socket(host, MESSAGE_PORT)) {
                    try (OutputStream os = s.getOutputStream(); PrintWriter out = new PrintWriter(os)) {
            ...
            public void run() {
                ...
                try {
                    ...
                    try (Socket s = new Socket(host, FILE_PORT)) {
                        ...
                        try (OutputStream os = s.getOutputStream(); PrintWriter out = new PrintWriter(os)) {
                            out.println(Config.LocalHostIP);
                    ...
                    try (Socket s = new Socket(host, FILE_PORT)) {
                        ...
                        try (OutputStream os = s.getOutputStream(); InputStream is = new BufferedInputStream(new FileInputStream(file), BUFFER_SIZE)) {
                ...
                try (ServerSocket server = new ServerSocket(MESSAGE_PORT)) {
                    while (true) {
                        try (Socket incoming = server.accept()) {
                            try (InputStream is = incoming.getInputStream(); Scanner in = new Scanner(is)) {
     ...
     */
    public static List < String > getWlanIP() {
        try {
